...
MOCK_MATCH_RESULT_1 = MatchResult(
    product_id="P123",
    frame_id="F456",
    match_score=0.9,
...
MOCK_MATCH_RESULT_2 = MatchResult(
    product_id="P123",
    frame_id="F456",
    match_score=0.8,
    bounding_box=[150.0, 150.0, 250.0, 250.0],
...
async def test_multiple_products_in_single_frame_edge_case():
    """
    Tests the integration flow when the matcher service finds multiple matches for the same product
    in a single frame. The handler should publish all results and acknowledge the message.
    """
    ...
    # Mock the core matcher service to return multiple matches
    with patch('handlers.match_request_handler.matcher_service.match', new_callable=AsyncMock) as mock_match:
        mock_match.return_value = [MOCK_MATCH_RESULT_1, MOCK_MATCH_RESULT_2]
        ...
        with patch('handlers.match_request_handler.publisher') as mock_publisher_instance:
            ...
            expected_payload = {
                "product_id": "P123",
                "frame_id": "F456",
                "matches": [MOCK_MATCH_RESULT_1.model_dump(), MOCK_MATCH_RESULT_2.model_dump()]
            }
