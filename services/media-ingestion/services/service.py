...
class MediaIngestionService:
    """Main service class for media ingestion"""
    ...
    def __init__(self, db: DatabaseManager, broker: MessageBroker, data_root: str):
        ...
        self.ingestion = VideoIngestion(data_root)
    ...
    async def handle_videos_search_request(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            for platform in platforms:
                ...
                if platform_queries:
                    if platform == "youtube":
                        videos = await self.ingestion.search_youtube_videos(platform_queries, recency_days)
                        all_videos.extend(videos)
                    elif platform == "bilibili":
                        videos = await self.ingestion.search_bilibili_videos(platform_queries, recency_days)
                        all_videos.extend(videos)
    ...
    async def process_video(self, video_data: Dict[str, Any], job_id: str):
        ...
        try:
            ...
            keyframes = await self.ingestion.extract_keyframes(video_data["url"], video.video_id)
