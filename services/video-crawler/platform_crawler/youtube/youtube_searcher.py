...
class YoutubeSearcher:
    ...
    async def search_youtube(self, query: str, recency_days: int, num_ytb_videos: int) -> List[Dict[str, Any]]:
        ...
        while len(final_videos) < num_ytb_videos and attempts < MAX_ATTEMPTS:
            attempts += 1
            logger.info(
                f"Attempt {attempts}/{MAX_ATTEMPTS}: Searching YouTube for query: '{query}' "
                f"with recency_days: {recency_days}, num_ytb_videos: {num_ytb_videos} "
                f"(current search limit: {current_search_limit})")
