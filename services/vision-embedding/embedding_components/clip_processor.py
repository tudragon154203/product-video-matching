...
class CLIPProcessor:
    ...
    async def extract_clip_embeddings(self, image: Image.Image) -> Tuple[np.ndarray, np.ndarray]:
    ...
    async def extract_embeddings_with_mask(self, image_path: str, mask_path: str, img_size: Tuple[int, int]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        try:
            ...
            if image.size != img_size:
                image = image.resize(img_size, Image.LANCZOS) # Using LANCZOS for better quality resizing
