...
class TestBasicVisionEmbeddingService:
    ...
    async def test_product_image_ready_handler(self, mock_service):
        ...
        mock_service.handle_products_image_ready.assert_called_once_with(
            event_data
        )
    ...
    async def test_batch_handler(self, mock_service):
        ...
        batch_assert = mock_service.handle_products_images_masked_batch
        batch_assert.assert_called_once_with(event_data)

    @pytest.mark.asyncio
    async def test_error_handling(self, mock_service):
        """Test error handling in event handlers."""
        # Make service raise an exception
        mock_service.handle_products_image_ready.side_effect = Exception(
            "Test error"
        )
