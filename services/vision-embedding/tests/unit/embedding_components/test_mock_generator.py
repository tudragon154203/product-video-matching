...
class TestMockGenerator:
    ...
    def mock_generator(self):
        ...
        with patch(
            "embedding_components.mock_generator.MockEmbeddingGenerator"
        ) as mock_gen:
            instance = Mock()
    ...
    async def test_mock_generator_extract_embeddings(self, mock_generator):
        ...
        embeddings = await mock_generator.extract_embeddings(
            test_image_path
        )

        mock_generator.extract_embeddings.assert_called_once_with(
            test_image_path
        )
