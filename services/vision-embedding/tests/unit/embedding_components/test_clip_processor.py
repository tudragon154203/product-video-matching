...
class TestCLIPProcessor:
    ...
    def mock_clip_processor(self):
        ...
        with patch('embedding_components.clip_processor.CLIPProcessor') as mock_processor:
            instance = Mock()
            instance.initialize = AsyncMock()
            instance.extract_embeddings = AsyncMock(return_value=np.array([0.1, 0.2, 0.3]))
    ...
    @pytest.mark.asyncio
    async def test_clip_processor_extract_embeddings(self, mock_clip_processor):
        """Test CLIP processor embedding extraction."""
        test_image_path = "/tmp/test_image.jpg"
        embeddings = await mock_clip_processor.extract_embeddings(test_image_path)
        
        mock_clip_processor.extract_embeddings.assert_called_once_with(test_image_path)
