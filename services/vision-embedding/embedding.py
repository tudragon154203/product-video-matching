...
from embedding_components.clip_processor import (
    CLIPProcessor as CustomCLIPProcessor,
)
...
class EmbeddingExtractor:
    """Extract visual embeddings using the configured CLIP model."""

    def __init__(self, model_name: str = "clip-vit-b32") -> None:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.clip_processor_instance = CustomCLIPProcessor(
                self.model,
                self.processor,
                self.device,
            )
    ...
    async def extract_embeddings(
        ...
        try:
            ...
            if self.initialized:
                ...
                processor = self.clip_processor_instance
                return await processor.extract_clip_embeddings(image)
    ...
    async def extract_embeddings_with_mask(
        ...
        """Extract embeddings from an image with the provided mask applied."""
        if self.initialized:
            processor = self.clip_processor_instance
            return await processor.extract_embeddings_with_mask(
                image_path,
                mask_path,
                config.IMG_SIZE,
            )
