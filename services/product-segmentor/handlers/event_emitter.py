...
class EventEmitter:
    ...
    async def emit_video_keyframes_masked(self, job_id: str, video_id: str, frames: List[dict]) -> None:
        ...
        try:
            ...
            logger.info("Successfully emitted video keyframes masked event", job_id=job_id,
                        video_id=video_id, frame_count=len(frames), event_id=event_id)
        except Exception as e:
            logger.error("Failed to emit video keyframes masked event", job_id=job_id,
                         video_id=video_id, frame_count=len(frames), event_id=event_id, error=str(e))
            raise
