...
class TestConcurrentProcessing:
    ...
    async def test_batch_completion_tracking(self, service_with_mock_segmentor):
        ...
        # Verify job progress manager was initialized
        job_tracking = service.job_progress_manager.job_tracking
        assert job_id in job_tracking
        assert job_tracking[job_id]["expected"] == total_images
        assert job_tracking[job_id]["done"] == 0
