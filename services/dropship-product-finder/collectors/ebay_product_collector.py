...
class EbayProductCollector(BaseProductCollector):
    ...
    def _initialize_browse_clients(self):
        ...
        try:
            for marketplace in self.marketplaces:
                ...
                if marketplace:
                    logger.info(f"Initializing browse client for marketplace: {marketplace}")
            ...
            logger.info(f"Initialized {len(self.browse_clients)} browse clients for marketplaces: {list(self.browse_clients.keys())}")
    ...
    async def close(self):
        ...
        for marketplace, client in self.browse_clients.items():
            try:
                # Note: Browse clients don't have explicit close methods in current implementation
                # but we should clean up any resources if they do in the future
        ...
        for marketplace in self.marketplaces:
            try:
                ...
                if marketplace not in self.browse_clients:
                    logger.warning(f"No browse client found for marketplace: {marketplace}")
                    continue
