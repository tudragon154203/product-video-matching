...
class MockProductCollector(IProductCollector):
    def __init__(
        self, products_to_return: List[Dict[str, Any]], should_fail: bool = False
    ):
...
class MockImageStorageManager:
    ...
    async def store_product(
        ...
        if self.should_fail and (
            ...
            or product.get("id") == self.fail_on_product_id
        ):
...
async def test_parallel_execution_basic(product_collection_manager):
    """Ensure Amazon and eBay workers run in parallel and return correct counts."""
...
async def test_collector_failure_handling(product_collection_manager):
    """Collector failures are handled without crashing the process."""
    ...
    amazon_stored = [
        p
        for p in product_collection_manager.image_storage_manager.stored_products
...
async def test_storage_failure_handling(product_collection_manager):
    """Storage failures are handled without crashing the process."""
...
async def test_empty_queries(product_collection_manager):
    ...
    assert ebay_count == 0
    assert (
        len(product_collection_manager.image_storage_manager.stored_products) == 0
    )
...
async def test_concurrency_verification():
    """Workers execute concurrently when measuring elapsed time."""
    ...
    async def slow_collect_products(query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        return [
            {"id": f"prod_{query}_{i}", "title": f"Product {i}"}
    ...
    # Since workers run in parallel, total time should be roughly the time of one
    # worker, not the sum of both workers. Each worker takes ~0.1s for
    # collection plus some storage time, so total should be < 0.3s (sequential
    # would be ~0.4s).
...
async def test_product_id_logging_in_storage_error():
    ...
    with patch("services.product_collection_manager.logger") as mock_logger:
        ...
        mock_storage_manager = MockImageStorageManager(
            should_fail=True, fail_on_product_id="test_prod_123"
        )
        manager = ProductCollectionManager(
            {
                "amazon": MockProductCollector(
                    [{"id": "test_prod_123", "title": "Test Product"}]
                ),
                "ebay": MockProductCollector([]),
            },
...
async def test_collector_logging_on_failure():
    ...
    with patch("services.product_collection_manager.logger") as mock_logger:
        ...
        manager = ProductCollectionManager(
            {"amazon": failing_collector, "ebay": MockProductCollector([])},
