...
async def test_e2e_flow():
    ...
    try:
        ...
        results["tests"]["auth"] = {
            ...
            "expires_in": stored_token.get("expires_in", "N/A") if stored_token else "N/A",
            "stored_at": stored_token.get("stored_at", "N/A") if stored_token else "N/A"
        }
    ...
    try:
        ...
        results["tests"]["refresh"] = {
            ...
            "new_expires_in": new_stored_token.get("expires_in", "N/A") if new_stored_token else "N/A"
        }
    ...
    try:
        ...
        results["tests"]["collection"] = {
            ...
            "products_per_second": round(len(collected_products) / collection_time, 2) if collection_time > 0 else 0
        }
        
        print(f"   ✅ Product collection successful")
        ...
        if collected_products:
            ...
            for i, product in enumerate(collected_products[:3]):
                print(f"     {i+1}. {product.get('title', 'N/A')} (ID: {product.get('id', 'N/A')})")
    ...
    try:
        ...
        if stored_token:
            ...
            print(f"   ✅ Redis integration successful")
            print(f"   Token stored in Redis: {len(json.dumps(stored_token))} bytes")
        else:
            results["tests"]["redis"] = {"status": "FAIL", "error": "No token found in Redis"}
            results["issues"].append("Redis integration failed: No token found")
            print(f"   ❌ No token found in Redis")
