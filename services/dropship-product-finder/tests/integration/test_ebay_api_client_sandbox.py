#!/usr/bin/env python3
"""Integration tests for EbayApiClient using eBay sandbox credentials."""
...
async def auth_service(redis_client):
    """Return a sandbox-configured eBay auth service."""
    if not config.EBAY_SANDBOX_CLIENT_ID or not config.EBAY_SANDBOX_CLIENT_SECRET:
        pytest.skip("Sandbox credentials are not configured")
...
async def ebay_browse_client(auth_service):
    """Instantiate the browse client bound to the sandbox marketplace."""
...
async def test_fetch_details_returns_multiple_images(ebay_api_client):
    """Fetch details via sandbox and ensure at least two image URLs are present."""
    query = "phone"
    limit = 5
    top_k = 3
