        ...
        for attempt in range(config.MAX_RETRIES_BROWSE):
            try:
                ...
                if not self.httpx_client:
                    await (
                        client.aclose()
                    )  # Close client if it was created within this method
                ...
                if response.status_code == 200:
                    ...
                    headers["Authorization"] = (
                        f"Bearer {await self.auth_service.get_token()}"
                    )
                    ...
                    logger.warning(
                        "Retry %s/%s waiting %ss",
                        attempt + 1,
                        config.MAX_RETRIES_BROWSE,
                        wait_time,
                    )
                ...
                last_error = e
                logger.error("Network error during request: %s", e)
                if attempt < config.MAX_RETRIES_BROWSE - 1:
                    wait_time = config.BACKOFF_BASE_BROWSE**attempt
                    logger.warning(
                        "Network error, retrying in %ss: %s", wait_time, e
                    )
                    await asyncio.sleep(wait_time)
                continue
