...
class PhaseEventService:
    ...
    async def handle_phase_event(self, event_type: str, event_data: Dict[str, Any]):
        ...
        logger.debug(f"Received event: {event_type} for job {event_data.get('job_id')}")
        ...
        if event_id in self.processed_events:
            logger.debug(f"Duplicate event, skipping: {event_id} for job {job_id}")
            return
        ...
        logger.debug(f"Stored phase event for job {job_id}: {event_type} (event_id={event_id})")
