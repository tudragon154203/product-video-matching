...
class TestPhaseTransitionWithAssetTypes:
    ...
    async def test_phase_transition_images_and_videos_job(self, phase_event_service, db_handler, mock_broker_handler):
        ...
        # Update stored events
        db_handler.stored_events[job_id] = {"image.embeddings.completed", "image.keypoints.completed"}
        ...
        # Update stored events
        db_handler.stored_events[job_id] = {"image.embeddings.completed", "image.keypoints.completed", "video.embeddings.completed"}
    ...
    async def test_phase_transition_to_evidences_generation(self, phase_event_service, db_handler, mock_broker_handler):
        ...
        db_handler.get_job_phase = AsyncMock(side_effect=["matching", "evidence"])
        db_handler.update_job_phase = AsyncMock()
    ...
    async def test_phase_transition_to_completed(self, phase_event_service, db_handler, mock_broker_handler):
        ...
        db_handler.get_job_phase = AsyncMock(side_effect=["evidence", "completed"])
        db_handler.update_job_phase = AsyncMock()
    ...
    async def test_phase_transition_failure_to_completed(self, phase_event_service, db_handler, mock_broker_handler):
        ...
        db_handler.get_job_counts = AsyncMock(return_value=(0, 0, 0))  # 0 products, 0 videos, 0 matches
        db_handler.get_features_counts = AsyncMock(return_value=(0, 0))  # 0 products with features, 0 videos with features
        
        db_handler.get_job_phase = AsyncMock(side_effect=["feature_extraction", "matching"])
        db_handler.update_job_phase = AsyncMock()
