...
async def test_gemini_integration():
    ...
    try:
        ...
        if not config.GEMINI_API_KEY:
            ...
            print(
                "To run this test successfully, please set GEMINI_API_KEY in your .env file.")
            return True  # Return True since this is an environment issue, not a code issue

        print(
            f"Loaded config: GEMINI_API_KEY={'*' * len(config.GEMINI_API_KEY) if config.GEMINI_API_KEY else 'NOT SET'}")
        ...
        cls_prompt = prompt_service.build_cls_prompt(
            test_query, industry_labels)
        ...
        try:
            ...
            try:
                ...
                if start != -1 and end > start:
                    ...
                    raise ValueError(
                        f"Could not extract JSON from response: {response_text}")

            # Try to print the generated queries, handling encoding issues
            try:
                print(
                    f"Generated queries: {json.dumps(queries, indent=2, ensure_ascii=False)}")
            except UnicodeEncodeError:
                # If there's an encoding issue, print with ascii encoding
                print(
                    f"Generated queries: {json.dumps(queries, indent=2, ensure_ascii=True)}")
