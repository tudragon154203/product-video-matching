    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if start_response.status_code != 200:
                print(
                    f"✗ Status endpoint test failed: Could not start job - HTTP {start_response.status_code}")
            ...
            if response.status_code == 200:
                ...
                required_fields = ["job_id", "phase",
                                   "percent", "counts", "updated_at"]
                missing_fields = [
                    field for field in required_fields if field not in data]

                if missing_fields:
                    print(
                        f"✗ Status endpoint test failed: Missing fields: {missing_fields}")
                    assert False, f"Missing fields: {missing_fields}"

                # Validate counts structure
                required_counts = ["products", "videos", "images", "frames"]
                missing_counts = [
                    count for count in required_counts if count not in data["counts"]]

                if missing_counts:
                    print(
                        f"✗ Status endpoint test failed: Missing counts: {missing_counts}")
                    assert False, f"Missing counts: {missing_counts}"

                # Validate data types
                assert isinstance(
                    data["job_id"], str), "job_id should be string"
                assert isinstance(data["phase"], str), "phase should be string"
                assert isinstance(
                    data["percent"], (int, float)), "percent should be number"
                assert 0 <= data["percent"] <= 100, "percent should be between 0-100"
                assert isinstance(
                    data["counts"], dict), "counts should be dict"

                # Validate counts are non-negative integers
                for count_type, count_value in data["counts"].items():
                    assert isinstance(
                        count_value, int), f"{count_type} count should be integer"
                ...
                print(
                    f"✗ Status endpoint test failed: HTTP {response.status_code}")
                try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                expected_conditions = (
                    ...
                    data.get("counts") == {"products": 0,
                                           "videos": 0, "images": 0, "frames": 0}
                )
                ...
                if expected_conditions:
                    ...
                    print(
                        "⚠ Status endpoint test for invalid job: Unexpected response for unknown job, but no error")
                ...
                print(
                    f"✗ Status endpoint test for invalid job failed: HTTP {response.status_code}")
                try:
                    error_data = response.json()
                    print(f"Error: {json.dumps(error_data, indent=2)}")
                except Exception:
