...
def setup_mocks(monkeypatch):
    ...
    video_crud_mock.list_videos_by_job.return_value = [
        MockVideo(MOCK_VIDEO_ID_1, "youtube", "url1", "Video Title 1",
                  120, datetime.now(timezone.utc) - timedelta(days=5)),
        MockVideo(MOCK_VIDEO_ID_2, "bilibili", "url2", "Another Video",
                  240, datetime.now(timezone.utc) - timedelta(days=10))
    ]
    ...
    video_frame_crud_mock.list_video_frames_by_video.return_value = [
        MockVideoFrame(MOCK_FRAME_ID_1, MOCK_VIDEO_ID_1, 10.5,
                       "/path/to/frame1.jpg", datetime.now(timezone.utc) - timedelta(days=1)),
        MockVideoFrame(MOCK_FRAME_ID_2, MOCK_VIDEO_ID_1, 20.0,
                       "/path/to/frame2.jpg", datetime.now(timezone.utc) - timedelta(days=2))
    ]
...
async def test_get_job_videos_includes_new_fields():
    ...
    mock_video = MockVideo(MOCK_VIDEO_ID_1, "youtube", "url1", "Video Title 1",
                           120, datetime.now(timezone.utc) - timedelta(days=5))
