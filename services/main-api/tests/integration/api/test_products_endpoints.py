    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if start_response.status_code != 200:
                print(
                    f"✗ Get job products test failed: Could not start job - HTTP {start_response.status_code}")
            ...
            if response.status_code == 200:
                ...
                if missing_fields:
                    print(
                        f"✗ Get job products test failed: Missing fields: {missing_fields}")
                    assert False, f"Missing fields: {missing_fields}"

                # Validate data types
                assert isinstance(
                    data["items"], list), "items should be a list"
                assert isinstance(
                    data["total"], int), "total should be an integer"
                assert isinstance(
                    data["limit"], int), "limit should be an integer"
                assert isinstance(
                    data["offset"], int), "offset should be an integer"
                ...
                if len(data["items"]) > 0:
                    ...
                    required_item_fields = [
                        "product_id", "src", "asin_or_itemid", "title", "brand", "url", "image_count", "created_at"]
                    missing_item_fields = [
                        field for field in required_item_fields if field not in item]

                    if missing_item_fields:
                        print(
                            f"✗ Get job products test failed: Missing item fields: {missing_item_fields}")
                        assert False, f"Missing item fields: {missing_item_fields}"

                    # Validate item field types
                    assert isinstance(item["product_id"],
                                      str), "product_id should be string"
                    assert isinstance(item["src"], str), "src should be string"
                    assert isinstance(
                        item["title"], str), "title should be string"
                    assert isinstance(
                        item["image_count"], int), "image_count should be integer"
                ...
                print(
                    f"✗ Get job products test failed: HTTP {response.status_code}")
                try:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if missing_fields:
                    print(
                        f"✗ Get job products test for invalid job failed: Missing fields: {missing_fields}")
                    assert False, f"Missing fields: {missing_fields}"
                ...
                print(
                    "✓ Get job products endpoint test for invalid job passed (404 response)")
                assert True
            else:
                print(
                    f"✗ Get job products test for invalid job failed: HTTP {response.status_code}")
                try:
                    error_data = response.json()
                    print(f"Error: {json.dumps(error_data, indent=2)}")
                except:
...
async def test_get_job_products_with_filters():
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if start_response.status_code != 200:
                print(
                    f"✗ Get job products with filters test failed: Could not start job - HTTP {start_response.status_code}")
                assert False, f"Could not start job - HTTP {start_response.status_code}"
            ...
            if not job_id:
                print(
                    "✗ Get job products with filters test failed: No job_id in start job response")
                assert False, "No job_id in start job response"
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✗ Get job products with filters test failed: HTTP {response.status_code}")
                try:
                    error_data = response.json()
                    print(f"Error: {json.dumps(error_data, indent=2)}")
                except:
