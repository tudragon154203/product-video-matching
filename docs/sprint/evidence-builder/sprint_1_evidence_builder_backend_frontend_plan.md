...
## 1) Purpose
- Stabilize the evidence-builder so it reliably consumes `match.result`/`match.request.completed`, renders deterministic artifacts, and only emits `evidences.generation.completed` when all matches are covered (including zero-match jobs).
...
- Entry: `services/evidence-builder/main.py` subscribes to `match.result` and `match.request.completed` and loops forever. Prefetch is set (10 for per-asset, 1 for completion).
...
- Service flow (`services/service.py`):
  ...
  - Zero-match path: `handle_match_request_completed` delegates to publisher, which publishes completion when count == 0.
...
- Goals:
  ...
  3. Emit `evidences.generation.completed` only when **all** matches for the job have an evidence asset, or immediately when `match.request.completed` sees zero matches.
...
- Align handler signatures with broker (`async def handle_match_result(event_data, correlation_id)`), and attach `@validate_event("match_result")` / `@validate_event("match_request_completed")` in `handlers/evidence_handler.py`.
...
- [ ] Fix handler signature mismatch: accept `correlation_id` and validate events with `@validate_event("match_result")` / `@validate_event("match_request_completed")`.
