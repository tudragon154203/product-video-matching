...
- Publish contract-compliant events (**match.result**) and a terminal **match.request.completed** per job.
...
- `match.result` (per accepted product‑video pair):
  ...
  - `score ∈ [0,1]`, `ts`
- `match.request.completed` (once per job): `{ job_id, event_id }`.
...
8. Emit `match.request.completed { job_id, event_id }`.
...
- Produce: `match.result`, `match.request.completed`.
...
3. On completion, emit exactly one `match.request.completed { job_id, event_id }`.
...
- Contracts / Schemas: `match_request.json` minimal `{ job_id, event_id }` with `additionalProperties=false`; `match_result`, `match_request_completed` present.
